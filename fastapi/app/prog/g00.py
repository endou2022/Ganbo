...
import mysql.connector as mydb  #mariadbにアクセス
import datetime
...
def get_programs_daily(service_type:str=Form(None), service_id:int=Form(None),genre:int=Form(None),nav_day:str=Form(None),nav_time:int=Form(None),cookie: str = Header(None)):
	...
	submitしたQuery , Form データを辞書として受け取る方法を見つけることができなかったので、引数が長くなった
	...
	for index , service_id2 in enumerate(service_id_list):
		...
		for program in programs:
			...
			program_start_time = program['開始時刻'] # program_start_time は datetime
