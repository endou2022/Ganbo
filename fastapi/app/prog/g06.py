...
sql = ('INSERT INTO `programs` (`ID` ,`イベントID` ,`サービスID` ,`ネットワークID` ,`開始時刻` ,`放送時間` ,`終了時刻` ,`フリー` ,`番組名` ,`説明` ,`ジャンル番号` ,`拡張情報` ) '
	'                   VALUES (%s   , %s          , %s          , %s              , %s        , %s        , %s        , %s      , %s      , %s    , %s            , %s ) '
...
def reflesh_programs():
	...
	for channel in channels:
		...
		insert_lists , program_data = analize_programs(programs)	# mirakurun から得たJSON形式の番組情報をデータベースに入力できる形にする
		prog_num += len(insert_lists)
		chg_name += cahnge_save_name(program_data)					# 番組名が変更されたら保存ファイル名も変更する
		cur.executemany(sql , insert_lists)							# 番組情報をデータベースに格納する
	...
	result['prog_num'] = prog_num	# alert()用
	result['chg_name'] = chg_name
	result['chg_prog'] = chg_prog

	logging.info(f'番組情報 = {result['番組情報']} , 更新件数 = {result['更新件数'] } , 名前変更 = {result['名前変更']}')
	return result
# --------------------------------------------------
...
def analize_programs(programs):
	...
	return insert_list , program_data
#---------------------------------------------------------------------------
